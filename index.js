...
function getFullNames(runners) {
  let arr = [];
  runners.forEach(bebop => arr.push(`${bebop.last_name}, ${bebop.first_name}`));
  return arr;
}
...
function firstNamesAllCaps(runners) {
  return runners.map(bebop => bebop.first_name.toUpperCase());
}
...
*/
function tallyUpDonations(run) {
  return run.reduce((sum, bebop) => sum + bebop.donation, 0);
}
